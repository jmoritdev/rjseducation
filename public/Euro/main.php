<?php   
    ...
    function genereerGeldstukken() {
		...
		function toonGeldstukken() {
				for($i = 0; $i < $_SESSION['aantalGeldStukken']; $i++){ 		// 4, 5 of 6 keer loopen
					...
					echo "<li style=\"display: inline;\"> <img src=\"" . $_SESSION['pic'][$geldstuk] . "\" width=\"250\" height=\"250\"> 
    ...
    function echoButton($i) {
      echo "<input type=\"submit\" name=\"antwoord$i\" value=\"" . number_format($_SESSION['alleAntwoorden'][$i],2) . "\"/>" ;    
    }
    
    function echoColorButton($i, $color) {
      echo "<input type=\"submit\" name=\"antwoord$i\" value=\"" . number_format($_SESSION['alleAntwoorden'][$i],2) . "\" style=\"border-color: $color\" />" ;    
    }
